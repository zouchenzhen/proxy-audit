...
- 在线入口改为开箱即用的 Hugging Face Docker Space，不再要求用户先启动本地引擎
...
- 保留完整本地版；Cloudflare Pages 指向 HF 的静态跳转包已准备好，待 HF 解封验收后部署
...
- 清理 README 中仅适用于维护者本机的结果路径和样本描述
...
- 从本机脱敏 raw 文件恢复最近任务和结果，默认显示 10 条、支持设置 1–100 条且不删除旧报告
- 页面刷新自动恢复运行中任务或最近结果
